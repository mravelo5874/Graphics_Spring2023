...
export class app
{
  ...
  // for canvas resizing
  private canvas_to_disp_size: Map<HTMLCanvasElement, number[]>
  ...
  private draw_scene(time: number): void
  {
    console.log('context: ' + this.context)
    let gl = this.context
  ...
  private resize_canvas_to_display_size(canvas) 
  {
    ...
    const [displayWidth, displayHeight] = this.canvas_to_disp_size.get(canvas) as number[];
