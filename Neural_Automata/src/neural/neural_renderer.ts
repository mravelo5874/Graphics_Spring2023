...
export class neural_renderer
{
    ...
    private one_pixel_location
    private kernel_location
    private color_mask_location
    ...
    public set_activation(_activation)
    {   
        this.activation_function = _activation
    }

    public set_kernel(_kernel)
    {
        this.kernel = _kernel
    }

    public set_color(_color: Vec3)
    {
    ...
    public compile_shaders(_vertex_source, _fragment_source)
    {
		let gl = this.gl
    ...
    public start_render()
    {
        if (this.is_running) throw 'called start_render() while already rendering'
        this.is_running = true
        this.apply_values()
        this.render()
	}

	private apply_values()
    {
		let gl = this.gl
	...
	public stop_render()
    {
		...
		if (this.updaterequest) clearTimeout(this.updaterequest)
	}
